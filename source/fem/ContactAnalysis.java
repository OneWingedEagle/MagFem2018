...
public class ContactAnalysis {
	...
	public Vect solve(Model model1, SpMatSolver solver,SpMat Khat1,Vect bhat1,int step) {
			...
			for (int load_iter = 0; load_iter < nLoads; load_iter++) {
				...
				slide_err.el[cont_iter] = slide.abs().max();
					...
					if (plot_radial)
					 ...
					 //if(gap_err.el[cont_iter]<1e-6 && (slide_err.el[cont_iter]<gap_tol))
					//	 break;
	...
	private void obtain_node_node3D() {
		...
		for (int contId = 0; contId < numContacts; contId++) {
			...
			for (int i = 0; i < slaveNodes[contId].length; i++) {
				...
				for (int k = 0; k < masterFacets[contId].length; k++) {
					...
					if (Math.abs(1 - sum / facetArea) > 1e-3) {
		...
		/*			int nnn = -1;
					if (v1v.norm() < 1e-5)
						nnn = 0;
					else if (v2v.norm() < 1e-5)
						nnn = 1;
					else if (v3v.norm() < 1e-5)
						nnn = 2;
					else if (v4v.norm() < 1e-5)
						nnn = 3;*/
					...
					/*
					 * if(pen<-100*edgeLength) { //weakenning.el[p]=0;
					 * 
					 * contacting[sn]=false;
					 * 
					 * continue; }
