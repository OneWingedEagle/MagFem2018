...
public class ContactAnalysis {
	...
	//	int nout=2601-2551+1;
		...
		for(int i=0;i<slaveNodes[contId].length;i++){
			...
			if(xind>=0)
			weights.el[xind]=1;//penFactor[contId]*util.max(val1,val2);			
			else if(yind>=0)
				weights.el[yind]=1;//penFactor[contId]*util.max(val1,val2);			
		}
	...
	//xr.show();
				...
				if(err.el[cont_iter]<1e-7 && (errf.el[cont_iter]<1e-7)) break;
		...
		if(plot){
		double[][] data=new double[xr.length][num_augs_run+1];
		for(int k=0;k<xr.length;k++){

			data[k][0]=xr.el[k];

			for(int i=0;i<num_augs_run;i++)
	...
	private void obtain_node_node(){
		...
		for(int i=0;i<slaveNodes[contId].length;i++){
			...
			for(int k=0;k<masterEdges[contId].length;k++){
				...
				if(pen>0e-12 && (!contacting[sn] || !check_double)) {
			...
			///	util.ph("normal.dot(edgeDir)   "+normal.dot(edgeDir));
				...
				//Vect tang=edgeDir.deepCopy();//delaDisp.normalized();
				//if(sld1.norm2()==0)
...
//util.pr(weights.el[com_index]);
				...
				if(p1x>=0){
					Gc.row[p].index[kx]=p1x;;
					Gc.row[p].el[kx++]=-alpha*tang.el[0];
				}
				if(p1y>=0){
					Gc.row[p].index[kx]=p1y;;
					Gc.row[p].el[kx++]=-alpha*tang.el[1];
				}
				
				if(p2x>=0){
					Gc.row[p].index[kx]=p2x;;
					Gc.row[p].el[kx++]=-beta*tang.el[0];
				}
				if(p2y>=0){
					Gc.row[p].index[kx]=p2y;;
					Gc.row[p].el[kx++]=-beta*tang.el[1];
				}
